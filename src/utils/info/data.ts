...
export function updateNameData(_data: BasicData): BasicData | undefined {
  let data = { ..._data };
  if (
    data?.token0?.id &&
    ...
    data.token0 = {
      ...data.token0,
      name: TOKEN_OVERRIDES[data.token0.id].name,
      symbol: TOKEN_OVERRIDES[data.token0.id].symbol,
    };
  }
  ...
  if (
    ...
    data.token1 = {
      ...data.token1,
      name: TOKEN_OVERRIDES[data.token1.id].name,
      symbol: TOKEN_OVERRIDES[data.token1.id].symbol,
    };
  }
